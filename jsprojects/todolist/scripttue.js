...
function handleTodoDelBtnClick(clickedId){
  todoArr1 = todoArr1.filter(function(aTodo){
...
function handleTodoItemClick(clickedId){
  todoArr1 = todoArr1.map(function(aTodo){
    return aTodo.todoId !== clickedId ? 
...
// saveTodos 함수
function saveTodos(){
  const todoSting = JSON.stringify(todoArr1)
  localStorage.setItem('myTodos1', todoSting)
}

// loadTodos 함수
function loadTodos(){
  const myTodos1 = localStorage.getItem('myTodos1') 
  todoArr1 = myTodos1 !== null ? JSON.parse(myTodos1) : todoArr1
  displayTodos()
}
