...
function handleTodoDelBtnClick(clickedId){
  todoArr3 = todoArr3.filter(function(aTodo){
...
function handleTodoItemClick(clickedId){
  todoArr3 = todoArr3.map(function(aTodo){
    return aTodo.todoId !== clickedId ? 
...
// saveTodos 함수
function saveTodos(){
  const todoSting = JSON.stringify(todoArr3)
  localStorage.setItem('myTodos3', todoSting)
}

// loadTodos 함수
function loadTodos(){
  const myTodos3 = localStorage.getItem('myTodos3') 
  todoArr3 = myTodos3 !== null ? JSON.parse(myTodos3) : todoArr3
  displayTodos()
}
