...
function handleTodoDelBtnClick(clickedId){
  todoArr4 = todoArr4.filter(function(aTodo){
...
function handleTodoItemClick(clickedId){
  todoArr4 = todoArr4.map(function(aTodo){
    return aTodo.todoId !== clickedId ? 
...
// saveTodos 함수
function saveTodos(){
  const todoSting = JSON.stringify(todoArr4)
  localStorage.setItem('myTodos4', todoSting)
}

// loadTodos 함수
function loadTodos(){
  const myTodos4 = localStorage.getItem('myTodos4') 
  todoArr4 = myTodos4 !== null ? JSON.parse(myTodos4) : todoArr4
  displayTodos()
}
