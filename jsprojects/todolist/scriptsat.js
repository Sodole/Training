...
function handleTodoDelBtnClick(clickedId){
  todoArr5 = todoArr5.filter(function(aTodo){
...
function handleTodoItemClick(clickedId){
  todoArr5 = todoArr5.map(function(aTodo){
    return aTodo.todoId !== clickedId ? 
...
// saveTodos 함수
function saveTodos(){
  const todoSting = JSON.stringify(todoArr5)
  localStorage.setItem('myTodos5', todoSting)
}

// loadTodos 함수
function loadTodos(){
  const myTodos5 = localStorage.getItem('myTodos5') 
  todoArr5 = myTodos5 !== null ? JSON.parse(myTodos5) : todoArr5
  displayTodos()
}
